...
public class ExperimentRunner {
    ...
    public static final double SAMPLE_PARAM = 200;
    public static final Long RANDOM_SEED = 12345L; // 可以设置为null表示纯随机
    public static final boolean USE_DYNAMIC_THRESHOLD = false;
    public static final boolean VERBOSE = false;
    public static final ErrorMeasure MEASURE = new G3Measure();
    
    // 数据集配置
    public static final boolean USE_DIRECTORY_MODE = false; // true: 使用目录模式, false: 使用硬编码列表模式
    public static final String DATASET_DIRECTORY = "data/0"; // 目录模式：指定数据集目录
    // 硬编码列表模式：指定数据集列表
    ...
    // 实验控制配置
    public static final boolean RUN_TANE = true; // 是否运行TANE算法
    public static final SamplingMode SAMPLING_MODE = SamplingMode.RANDOM; // 采样方法选择
    
    public static final String RESULTS_FILE = "result/result0715_test.csv";
    public static final ResultManager.RunMode RUN_MODE = ResultManager.RunMode.APPEND;
    // 可以设置为APPEND或OVERWRITE
    public static final boolean USE_TIME_SUFFIX = false; // 是否在结果文件名后添加时间后缀
    public static final long ALGORITHM_TIMEOUT_MINUTES = 120; // 每个算法执行的超时时间，单位：分钟
    ...
    public static void main(String[] args) {
        ...
        if (USE_DIRECTORY_MODE) {
            try {
                datasetPaths = getDatasetPaths(DATASET_DIRECTORY);
            ...
            if (datasetPaths.isEmpty()) {
                System.err.println("警告: 在" + DATASET_DIRECTORY + "目录下没有找到任何文件，请检查路径是否正确。");
                return;
            }
        } else {
            datasetPaths = new ArrayList<>(DATASET_PATHS);
            System.out.println("列表模式：使用" + datasetPaths.size() + "个预定义数据集:");
            for (String path : datasetPaths) {
                try {
                    char delimiter = detectDelimiter(path);
                    int columns = getColumnCount(path);
